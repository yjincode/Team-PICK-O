...
//     console.log("🔊 FastWhisper 결과:", data.text)
//   } catch (err) {
//     console.error("STT 실패:", err)
//     setError('STT 실패: ' + (err instanceof Error ? err.message : String(err)))
//     onError?.('STT 실패')
//   } finally {
...
const AudioRecorderSTT: React.FC<Props> = ({ onTranscriptionComplete, onError }) => {
  const [isRecording, setIsRecording] = useState(false)
  const [isProcessing, setIsProcessing] = useState(false)
  ...
  const stt = async (file: File) => {
    ...
    try {
      ...
      console.error('STT 실패:', err)
      const msg = err instanceof Error ? err.message : 'STT 처리 중 오류 발생'
      setError(msg)
      onError?.(msg)
    } finally {
      setIsProcessing(false)
